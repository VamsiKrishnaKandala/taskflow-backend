...
public class GlobalExceptionHandler {
    ...
    private Mono<ResponseEntity<ErrorResponse>> buildErrorResponse(HttpStatus status, String error, String message) {
        ErrorResponse resp = new ErrorResponse(LocalDateTime.now(), status.value(), error, message);
        log.warn("Exception handled [{}]: {} - {}", status, error, message);
        return Mono.just(ResponseEntity.status(status).contentType(MediaType.APPLICATION_JSON).body(resp));
    }
    ...
    public Mono<ResponseEntity<ErrorResponse>> handleInvalidTaskData(InvalidTaskDataException ex) {
    ...
    public Mono<ResponseEntity<ErrorResponse>> handleUnauthorized(UnauthorizedActionException ex) {
        return buildErrorResponse(HttpStatus.FORBIDDEN, "Unauthorized Action", ex.getMessage());
    }

    @ExceptionHandler(TaskEventPublishingException.class)
    public Mono<ResponseEntity<ErrorResponse>> handleTaskEventPublishing(TaskEventPublishingException ex) {
        log.error("Event publishing failure: {}", ex.getMessage(), ex);
        return buildErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR, "Event Publishing Failed", ex.getMessage());
    }
    ...
    public Mono<ResponseEntity<ErrorResponse>> handleGenericError(Throwable ex) {
        log.error("Unhandled exception in task-service: {}", ex.getMessage(), ex);
